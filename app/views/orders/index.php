        ...
        <section class="bg-white shadow order-data rounded p-5 w-100 overflow-x-scroll">
            ...
            <form action="<?= BASEURL ?>orders/search" method="post" class="mt-4">
                <div class="input-group mb-3 z-0">
                    <input type="text" class="form-control rounded-0 z-0" placeholder="Search Order"
                        aria-label="Search Order" aria-describedby="button-search" name="keyword" />
            ...
            ?>
                    <tr>
                        ...
                        <?= $order['paid_stat'] !== 'Paid' ? '<td class="fw-semibold table-danger text-danger">' . $order['paid_stat'] . '</td>' : '<td class="fw-semibold table-success text-success">' . $order['paid_stat'] . '</td>' ?>
                        <td><?= is_null($order['date_paid']) ? '-' : $order['date_paid'] ?></td>

                        <td class="table-primary">
                            <a data-bs-toggle="modal" data-bs-target="#detailModal" onclick="showDetail(this)"
                        ...
                        <td class="table-warning">
                            <a href="<?= BASEURL ?>orders/form/<?= $order['id_orders'] ?>"
                                class="text-decoration-none"><i class="text-warning fa-solid fa-pen"></i></a>
                        </td>
                        <?php else : ?>
                        <td class="table-warning">
    ...
    <div class="modal fade" id="deleteConfirmModal" tabindex="-1" aria-labelledby="deleteConfirmModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <a class="btn btn-danger" id="confirm-delete-btn" href="#">Delete</a>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                </div>
            </div>
        </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".delete-btn").forEach(button => {
                button.addEventListener("click", function(event) {
                    ...
                    // Ambil ID order dari atribut data-id
                    let orderId = this.getAttribute("data-id");

                    // Set URL delete ke tombol confirm di modal
                    let deleteUrl = "<?= BASEURL ?>orders/delete/" + orderId;
